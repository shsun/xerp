...
import base.utils.BaseCookieUtil;
...
//            }
        ...
        String filePath = context.getRealPath("/jsptag" + this.getModuleName() + "/config") + "/" + configFile;
        Element root = null;
    ...
    protected void init() {
        ...
        if (BaseStringUtil.isBlank(template)) {
            template = this.getModuleName() + "/ftl/default/" + this.getSimpleName() + ".ftl";
        } else {
            template = this.getModuleName() + "/ftl/" + template;
        }

        this.setTemplate(template.trim());

        String c = (String) configMap.get("cache");
        if (!BaseStringUtil.isBlank(c)) {
