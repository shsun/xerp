...
public class PageTag extends BodyTagSupport {
    ...
    private String rel;
    private String seq;  //seq>0 则 pageNo${seq}=* , 否则pageNo=*
    ...
    private String getPageNoName(){
        String pageNoName = "pageNo";
        if(BaseStringUtil.isNotBlank(this.seq)){
    ...
    private void println(JspWriter out, Object name, int page) throws IOException {
        ...
        String pageNoName = this.getPageNoName();
        ...
        if (page == pageNo) {
            ...
            if(BaseStringUtil.isBlank(this.urlString)){
                href = "?" + pageNoName + "=" + page + queryString;
            }else{

                //指定地址
                if(this.urlString.endsWith(".html")){
                   String str = "_" + page + ".html";
                    if(page <2){
                        str = ".html";
                    }
                    if(BaseStringUtil.isNotBlank(this.getAnchor())){
                        str = "#" + this.getAnchor();
                    }
                   href = this.urlString.replace(".html",str);
                }else{
                    String andStr = "&";
                    if(this.urlString.indexOf("?")==-1){
                      andStr = "?";
                    }
                    href = this.urlString+ andStr +  pageNoName + "=" + page;
                    if(BaseStringUtil.isNotBlank(this.getAnchor())){
                        href = "#" + this.getAnchor();
                    }
                }
    ...
    private String getQueryString2(String queryString) {
        if (queryString == null) {
            ...
            String pageNoName = this.getPageNoName();
            queryString = queryString.replaceAll("&" + pageNoName + "=\\d{0,}", "");
            queryString = queryString.replaceAll(pageNoName + "=\\d{0,}", "");
        }
