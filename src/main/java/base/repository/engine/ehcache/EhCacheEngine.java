...
public class EhCacheEngine implements CacheEngine {
     ...
     */
	@SuppressWarnings("deprecation")
	public Collection<Serializable> getValues(String fullyQualifiedName) {
		try {
			if (!manager.cacheExists(fullyQualifiedName)) {
				synchronized (EhCacheEngine.class) {
					if (!manager.cacheExists(fullyQualifiedName)) {
			...
			Cache cache = manager.getCache(fullyQualifiedName);
			List<Serializable> values = new ArrayList<Serializable>(cache.getSize());
	 ...
	 * @return  Map
	 */
	public Map<Serializable, Element> getMap( String fullyQualifiedName ) {
		try {
			if (!manager.cacheExists(fullyQualifiedName)) {
				synchronized (EhCacheEngine.class) {
					if (!manager.cacheExists(fullyQualifiedName)) {
				...
				return new HashMap<Serializable, Element>();
			}
			Cache cache = manager.getCache(fullyQualifiedName);
			Map<Serializable, Element> map = new HashMap<Serializable, Element>( cache.getSize() );
			List<?> keys = cache.getKeys();
	...
	private void sortValues( @SuppressWarnings("rawtypes") List valueList ) {
