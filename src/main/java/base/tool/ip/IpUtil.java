...
public class IpUtil {
    ...
    private Hashtable<String, IPLocation> ipCache;// 用来做为cache，查询一个ip时首先查看cache，以减少不必要的重复查找
...
//        ////System.out.println( new IpUtil().getClass().getClassLoader().getResource(".").getPath());
     ...
     */
    public List<IpBean> getIPEntriesDebug(String s) {
        List<IpBean> ret = new ArrayList<IpBean>();
     ...
     * @return 包含IpBean类型的List
     */
    public List<IpBean> getIPEntries(String s) {
        List<IpBean> ret = new ArrayList<IpBean>();
        try {
            // 映射IP信息文件到内存中
            if (mbb == null) {
     ...
     * @return String
     */
    public String getClassPath(Class<? extends IpUtil> clazz) {
