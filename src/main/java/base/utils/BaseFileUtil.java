...
public class BaseFileUtil {
    ...
    public static File getFileDir(String pathDir){
    ...
    public static void main(String[] args) throws Exception {
        String zipFileName = "C:\\a\\a.zip";
        String extPlace = "C:\\a";
        File file = extZipFile(zipFileName,extPlace);
    ...
    public static void copyFromUrl(String host, String targetPath, String targetName) throws Exception {
        try {
        ...
        copyFile(input, targetPath, targetName);
    }

    public static void copyFromUrl(String host, String targetPath) throws Exception {
        int i = host.lastIndexOf("/");
        String targetName = host.substring(i+1);
        copyFromUrl(host,targetPath,targetName);
    }

    public static void copyFromUrl(String host, String targetPath,boolean replace) throws Exception {
        int i = host.lastIndexOf("/");
        String targetName = host.substring(i+1);
        if(!replace){
     ...
     * @throws Exception
     */
    public static void copyFile(InputStream input, String targetPath, String targetName) {
        File targetFile = null;
        try {
            targetFile = new File(targetPath + File.separator + targetName);
            FileOutputStream fileOutputStream = new FileOutputStream(targetFile);
            byte[] buf = new byte[1024];
            int i = 0;
     ...
     * @return
     */
    public static int getNumFile(String fileDirectory){
...
//     cd.setTimeInMillis(time);
    ...
    //--------------------------最新的---------------------------------
     ...
     public static long getDirSize(File file) {
        ...
        public static String getFromPathOrUrl(String pathOrUrl) {
        if (BaseStringUtil.isBlank(pathOrUrl)) {
            return "";
        }

        //从网址取数据
        if (pathOrUrl.trim().startsWith("http")) {
            return BaseUrlUtil.download(pathOrUrl, "UTF-8");
        }

        //从本地文件取
        File jsonFile = new File(pathOrUrl);
        if (!jsonFile.exists()) {
            return "";
        }
