angular.module('myApp').controller('DepartmentCtrl', function($scope, $uibModal, $http, $anchorScroll, confirmDialogs){
	...
	$scope.search = function() {
		$anchorScroll();
		$scope.promise = $http.post(ctx + '/admin/DepartmentCtrl/search', $scope.searchForm).success(function(data){
			$scope.dataList = data;
		});
	};
	...
	$scope.edit = function(item) {
		...
		$uibModal.open({
			templateUrl: ctx + '/templates/admin/DepartmentEditTpl.html?v=',
	...
	$scope.del = function(item) {
		...
		.result.then(function (btn) {
			if(btn == 'ok') {
				$scope.promise = $http.get(ctx + '/admin/DepartmentCtrl/delete?id='+item.id).success(function(data){
...
angular.module('myApp').controller('DepartmentEditCtrl', function($scope, $http, $uibModalInstance, confirmDialogs,id){
	$scope.editForm = {};
	$scope.editForm.id = id;
	if(id && id > 0) {
		$scope.title = '编辑';
		$scope.promise = $http.get(ctx + '/admin/DepartmentCtrl/getDetailInfo?id='+id).success(function(data){
	...
	$scope.save = function() {
		$scope.promise = $http.post(ctx + '/admin/DepartmentCtrl/save', $scope.editForm).success(function(data){
