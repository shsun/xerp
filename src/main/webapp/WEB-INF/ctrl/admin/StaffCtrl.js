angular.module('myApp').controller('StaffCtrl', function($scope, $uibModal, $http, $anchorScroll, confirmDialogs){
	...
	$scope.search = function() {
		$anchorScroll();
		$scope.promise = $http.post(ctx + '/admin/StaffCtrl/search', $scope.searchForm).success(function(data){
			$scope.dataList = data;
		});
	};
	...
	$scope.edit = function(item) {
		...
		$uibModal.open({
			templateUrl: ctx + '/templates/admin/StaffEditTpl.html?v=',
	...
	$scope.del = function(item) {
		...
		.result.then(function (btn) {
			if(btn == 'ok') {
				$scope.promise = $http.get(ctx + '/admin/StaffCtrl/delete?id='+item.id).success(function(data){
...
angular.module('myApp').controller('StaffEditCtrl', function($scope, $http, $uibModalInstance, confirmDialogs,id){
	$scope.editForm = {};
	...
	$scope.save = function() {
		$scope.promise = $http.post(ctx + '/admin/StaffCtrl/save', $scope.editForm).success(function(data){
